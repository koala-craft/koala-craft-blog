...
- **Site URL**: アプリのベース URL（開発: `http://localhost:3000`、本番: `https://your-domain.com`）
- **Redirect URLs**: 上記に加え、`http://localhost:3000/**`、`https://your-domain.com/**` 等を追加
- ワイルドカードを過度に緩くしない（例: `https://*.vercel.app` は必要に応じて）

ログイン後、Supabase は Site URL または Redirect URLs に一致する URL へリダイレクトする。管理画面は `/admin` なので、`/admin` を含むパスが許可されていれば問題ない。
